...
/interface vrrp
add interface=ether2 name=vrrp1 preemption-mode=no priority=110 \
    remote-address=127.1.1.2 sync-connection-tracking=yes
add comment=vlan4 interface=vlan4 name=vrrp2 preemption-mode=no priority=110 \
    remote-address=10.4.0.3 sync-connection-tracking=yes version=2
/interface list
add name=WAN
add name=LAN
...
/ip address
add address=10.4.0.2/24 interface=vlan4
add address=10.4.0.1/24 interface=vrrp2
add address=1.1.1.1/30 interface=vrrp1
add address=127.1.1.1/30 interface=ether2
...
/routing ospf
instance add name=ospf-instance-1 out-filter-chain=ospf router-id=0.0.0.1
